...
>
> 当前状态：**MVP 全部模块已实现，前端 13 页面、后端 8 模块完整运行**
...
| M2 | 错题录入 | P0 | ✅ 完成 | AI 视觉模型 OCR + 手动输入，支持 Markdown+LaTeX |
| M3 | 错题管理 | P0 | ✅ 完成 | 列表/搜索/多维筛选/编辑/删除/批量删除 |
| M4 | AI 错因分析 | P0 | ✅ 完成 | 5 类错因自动分析，前端饼图统计 |
| M5 | 变体题生成 | P1 | ✅ 完成 | AI 生成 3 道同类题，Tool Use 计算器加速，前端 KaTeX 渲染，支持在线作答评判 |
...
| **四项能力** | ① 错因分析（5类自动归类） ② 变体题生成（3道+解析） ③ 答案评判（正确/错误） ④ Tool Use 工具调用（计算器加速复杂计算） |
...
| 5 | Docker 容器化 | ✅ 已完成 | 环境一致性 | docker-compose.yml 已就绪 |
